...
import { Address, beginCell, Builder, Cell, ContractProvider, Dictionary, DictionaryValue, Slice } from "@ton/core";
...
export const getLastBlock = async (provider: NetworkProvider) => {
  const api =  await provider.api() as TonClient4;
  return  await api.getLastBlock();
}
export const getAccountLastTx = async (provider: NetworkProvider, address: Address) => {
    const api =  await provider.api() as TonClient4;
    const res = await api.getAccountLite((await getLastBlock(provider)).last.seqno, address);
    if(res.account.last == null)
        throw(Error("Contract is not active"));
    return res.account.last.lt;
}
export const waitForTransaction = async (contractProvider:ContractProvider, address:Address, curTx:bigint | null, maxRetry:number, interval:number=1000) => {
    let done  = false;
    let count = 0;
    ...
    do {
        ...
        console.log(JSON.stringify(curState.last?.lt.toString()));
        if(curState.last?.lt !== null){
            console.log("curTx", curState);
