...
export  async function transfer(provider:NetworkProvider, ui:UIProvider) {
    const sender = provider.sender();
    let retry:boolean;
    ...
    do {
        ...
        amount    = await promptAmount('Please specify amount to transfer:', ui);
        ui.write(`Transfer ${amount} to ${toAddress}\n`);
        retry = !(await promptBool('Is it ok?(yes/no)', ['yes', 'no'], ui));
    } while(retry);
    ...
    const nullCell = beginCell().endCell();
    const txRes = await jettonWallet.sendTransfer(
          sender,
          toNano('0.1'), 
          nanoAmount,
        toAddress, 
        sender.address!,
        nullCell , toNano('0.000000001'), nullCell);
        console.log(txRes);

    const gotTrans = await waitForTransaction(walletProvider,
                                              walletAddress,
                                              curState.last.lt,
                                              10);
    if(gotTrans) {
        const balanceAfter = parseInt((await jettonWallet.getJettonBalance()).toString());
        if(balanceAfter  > 0) {
            ui.write("Transfer successfull!\nCurrent balance:" + fromNano(balanceAfter));
        }
        else {
